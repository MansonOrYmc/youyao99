...
abstract class APF_DB_DataObject implements ArrayAccess
{
    ...
    /**
     * @var array 存储数据库刚取出时的数据
     */
    private $old_property_values;
    ...
    public function add_old_property_value($name, $value)
    {
        $this->old_property_values[$name] = $value;
    }
    ...
    protected function insert() {
        ...
        foreach ($this->properties() as $property) {
            if (!is_null($this->$property)){
                ...
                $this->add_old_property_value($property, $this->$property);
            } else {
                $this->add_old_property_value($property, null);
            }
        }
        ...
        $this->add_old_property_value($key, $this->$key);
    ...
    protected function update() {
        ...
        foreach ($this->properties() as $property) {
            //只有更新过的数据才纳入更新
            if (!is_null($this->$property) && ($this->$property != $this->old_property_values[$property]) ){
                $update_needed = true;
                $accessor->set_field($property, $this->$property);
                $this->add_old_property_value($property, $this->$property);
            }
        ...
        $affected_rows = 0;
        if($update_needed){
            $affected_rows = $accessor->filter($this->key(), $this->get_primary_key())->update();
        }
    ...
    {
        return $this->checkPropertyExists($property, false);
    }

    public function offsetGet($property)
    {
        $this->checkPropertyExists($property, true);
    ...
    private function checkPropertyExists($property, $throwException = false)
    {
        $isExist = in_array($property, $this->properties());
        if (!$isExist && $throwException) {
            throw new Exception("$property is not " . get_called_class() . "'s property.");
        }
