...
class APF_DB_MysqlAccessor  {
    ...
    protected function convert_obj_to_array($obj)
    {
        if (empty($this->load_fields)) {
            $properties = $this->get_obj_columns();
    ...
    protected function init_objs($rs)
    {
        ...
        foreach ($rs as $r) {
            ...
            foreach ($this->mapping['columns'] as $objColumn => $dbColumn) {
                ...
                $obj->add_old_property_value($objColumn, $obj->$objColumn);
            }
            $obj->isLoaded = true;
            $objs[] = $obj;
        }
    ...
    protected function get_obj_columns()
    {
        return array_keys($this->mapping['columns']);
    }
